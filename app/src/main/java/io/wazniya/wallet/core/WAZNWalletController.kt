...
import io.wazniya.wazn.data.TxData
import io.wazniya.wazn.model.*
import io.wazniya.wazn.util.RestoreHeight
...
object WAZNWalletController {
    ...
    fun createWallet(aFile: File, password: String): io.wazniya.wallet.data.entity.Wallet {
    ...
    private fun close(success: Boolean, newWallet: Wallet): io.wazniya.wallet.data.entity.Wallet {
        try {
            if (!success) {
                throw RuntimeException(newWallet.errorString)
            } else {
                val sym = sharedPreferences().getString("symbol", "") ?: ""
                return io.wazniya.wallet.data.entity.Wallet().apply {
                    symbol = sym
                    name = newWallet.name
    ...
    fun getTransactionHistory(): List<io.wazniya.wallet.data.entity.TransactionInfo> {
        val wallet = getWallet() ?: return emptyList()
        val all = wallet.history.all.sortedByDescending { it.timestamp }
        val allInfo = mutableListOf<io.wazniya.wallet.data.entity.TransactionInfo>()
        all.forEach {
    ...
    private fun convert(it: TransactionInfo): io.wazniya.wallet.data.entity.TransactionInfo {
        return io.wazniya.wallet.data.entity.TransactionInfo().apply {
    ...
    fun testRpcService(url: String, timeout: Int = 3 * 1000): Long {
        ...
        if (connection.responseCode == HttpURLConnection.HTTP_OK) {
            ...
            Log.e("wazniya", "$url--->>> height: ${header.optLong("height")}")
            Log.e("wazniya", "$url--->>> majorVersion: ${header.optLong("major_version")}")
        }
