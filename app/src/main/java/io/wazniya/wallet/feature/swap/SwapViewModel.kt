...
import io.wazniya.wallet.data.remote.entity.Empty
import io.wazniya.wallet.data.remote.entity.ExchangeAmount
import io.wazniya.wallet.data.remote.entity.RPCRequest
import io.wazniya.wallet.data.remote.entity.SwapCreateTransaction
...
class SwapViewModel : BaseViewModel() {
    ...
    private var observer = object : WAZNWalletController.Observer {
        var firstBlock = 0L
            ...
            try {
                ...
                minFromAmount.postValue(
                    RetrofitManager.changellyService.getMinAmount(
                        RPCRequest(
                            method = "getMinAmount",
                            params = mapOf("from" to "wazn", "to" to toCoin)
                        )
                    ).unWrap()
                )
                ...
                exchangeAmount.postValue(
                    RetrofitManager.changellyService.getExchangeAmount(
                        RPCRequest(
                            ...
                            params = listOf(mapOf("from" to "wazn", "to" to toCoin, "amount" to "1"))
                        )
                    ).unWrap()?.firstOrNull()
                )
            } catch (e: Exception) {
...
//                        saveTransaction()
                        ...
                        WAZNWalletController.startWallet(
                            path,
                            password,
    ...
    private fun updateBalance() {
        try {
            val b = WAZNWalletController.getBalance()?.let {
                WAZNWalletController.getDisplayAmount(it)
            }
            balance.postValue(b)
        ...
        if (!send) {
            try {
                swapCreateTransaction?.let {
                    WAZNWalletController.createTransaction(
                        false,
                        it.payinAddress,
                        null,
