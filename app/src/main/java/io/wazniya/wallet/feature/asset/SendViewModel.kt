...
import io.wazniya.wallet.support.REQUEST_SELECT_ADDRESS
...
class SendViewModel : BaseViewModel() {
    ...
    fun addressChanged(it: String) {
        if (it.isNullOrBlank()) {
            ...
            addressError.value = !WAZNWalletController.isAddressValid(it)
        }
    ...
    fun paymentIdChanged(it: String) {
        if (it.isNullOrBlank()) {
            ...
            paymentIdError.value = !WAZNWalletController.isPaymentIdValid(it)
        }
        paymentId.value = it
        if (it.length > 16) {
    ...
    fun clickGenerate() {
        uiScope.launch {
            withContext(Dispatchers.IO) {
                autoFillPaymentId.postValue(WAZNWalletController.generatePaymentId())
            }
        }
    }
    ...
    fun next() {
        ...
        uiScope.launch {
            try {
                withContext(Dispatchers.IO) {
                    WAZNWalletController.createTransaction(isAll, receiveAddress.value!!, paymentId.value, receiveAmount.value!!)
